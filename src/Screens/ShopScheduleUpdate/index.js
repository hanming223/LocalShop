...
import { vendor_update_shop_schedule } from "../../Components/Api";
...
export default class ShopScheduleUpdate extends Component {
    constructor(props) {
        super(props);
        this.state = ({
            data: [],
    ...
    async onDoneButton() {
        ...
        formData.append('shopSchedules', params);
        formData.append('manageShop', AppManager.getInstance.selectedShopInfo.id);

        let response = await vendor_update_shop_schedule(formData);

        if (response.result == true){
            Alert.alert(
                'Success!',
                'Shop schedule has been updated.',
                [
                  {text: 'OK', onPress: () => {
